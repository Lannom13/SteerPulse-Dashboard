...
export default function InsightsPanel({ category, onClose }) {
  ...
  useEffect(() => {
    const fetchMerchants = async () => {
      try {
        const res = await fetch('https://lpczocldblkfrhnlpqgf.functions.supabase.co/get_transactions_by_category', {
          method: 'POST',
  ...
  return (
    <div className="fixed top-0 right-0 w-96 h-full bg-gray-900 shadow-lg border-l border-gray-700 z-50 p-6 overflow-y-auto">
      ...
      {loading ? (
            ...
            <li key={idx} className="bg-gray-800 rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <div>
                  ...
                  <p className="text-sm text-gray-400">${m.amount.toFixed(2)} • {m.percent}%</p>
                </div>
        ...
        <p className="text-sm text-gray-400">No recent merchant activity found for this category.</p>
      )}
    </div>
  );
}

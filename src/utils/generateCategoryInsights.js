export function generateCategoryInsights(categoryData) {
  ...
  if (percent > 110) {
    ...
    insights.push(`👍 You're spending well under budget on "${category}". Could you shift some to other priorities?`);
  } else {
  ...
  if (difference > 0) {
    ...
    if (percent < 80) {
      insights.push(`💸 You might consider reallocating $${Math.abs(difference).toFixed(2)} to savings or debt payments.`);
    }
  }
